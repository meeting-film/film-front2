                    ...
                    <form ref="form-normal" class="form form-common" @submit.prevent="loginSubmit" autocomplete="off">
                        ...
                        <div class="form-field form-field--icon">
                            ...
                            <input name="username" v-model="form.username" v-validate="'required|username'"
                                   :class="{'form-control': true, 'is-danger': errors.has('username') }" type="text"
                                   placeholder="用户名">
                        ...
                        <p class="signup-guide">还没有账号？
                            <nuxt-link to="/register" class="link">免费注册</nuxt-link>
                        </p>
                    </form>
                    <!--普通方式登录 end-->
                    <!--登录表单 end-->
        ...
        methods: {
            loginSubmit() {
                ...
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        getData(process.env.BASE_URL + '/auth', 'post', params).then((res) => {
                            if (res) {
                                if (res.status == 1 && res.msg) {
                                    ...
                                    // Cookies.set('token', res.data.token, { expires: date, path: '' });
                                    // Cookies.set('username', params.userName, { expires: date, path: '' });
            ...
            .lc-form-wrapper__right {
                ...
                .btn-group {
                    .btn-login {
                        ...
                        background: -webkit-gradient(linear, left top, left bottom, color-stop(0, $baseColorHover), color-stop(100%, $baseColor));
                        background: -webkit-linear-gradient(top, $baseColorHover 0, $baseColor 100%);
                        background: linear-gradient(to bottom, $baseColorHover 0, $baseColor 100%);
                        filter: progid:DXImageTransform.Microsoft.gradient(startColorstr=$baseColorHover, endColorstr=$baseColor, GradientType=0);
                    }
                }
                .signup-guide {
                    padding: 5px 0;
